...
class FrontendController extends Controller
{
        ...
        try {
            ...
            if ($lang) {
                ...
                if ($sidebarData) {
                    ...
                    $allCollection = ProductsCollection::leftJoin('product_collection_langs as pcl',
                ...
                if ($sidebarData) {
                    ...
                    $allCollection = ProductsCollection::where('status', Config::get('constants.status.PUBLIC'))
                        ->select('id', 'title')
                        ->get();
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($request->required_rating) {
                $data['review'] = Products::where('admin_id', $store->admin_id)->avg('rating');
            }
    ...
    {
        $home = Http::get('https://globaldrinkshub.com/api/v1/home');
        return $home;
    }
    ...
    public function productSuggestion(Request $request, $id)
    {
        ...
        try {
            ...
            $product = Products::select('id')->find($id);
            $data['suggestion_1'] = [];
            $data['suggestion_2'] = [];

            if ($product) {

                $qSug1 = Products::query();
